...
def dec_to_degrees(dec):
	...
	return(minutes/60.0 + degrees + seconds/360.0)
...
if(len(sys.argv)!=5):
	...
	print "\tUseage: gsmfilter <filename> <flux cutoff> <radius> <outputname>"
...
for cluster_ctr in range(len(cluster_index)):
	...
	for ctr in range(len(cluster_index[cluster_ctr])):
		if(lines[cluster_index[cluster_ctr][ctr]].split(',')[type_col] == ' GAUSSIAN'):
			...
			f.write(" "+str(lines[cluster_index[cluster_ctr][ctr]].split(',')[name_col]))
	f.write("\n")
f.close()
